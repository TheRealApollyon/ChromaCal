...
Eight events came up as a real gap in calendar coverage: the four fixed cross-quarter days, Imbolc, Beltane, Lughnasadh, and Samhain, plus the four solstices and equinoxes. The four fixed ones are simple, they're just calendar dates like everything else already in the system, no different in kind from any other single-day holiday. All eight, plus the design questions below, shipped in v1 as a beta on `pagan-wiccan-beta`, genuinely live and tested, not just planned.

The solstices and equinoxes were the harder piece, since the actual moment shifts by a day or sometimes two each year depending on that year's specific orbital timing, not a fixed date the way Christmas or Independence Day are. For v1, a hardcoded lookup table was the right call, consistent with how the rest of the floating-date problem already gets solved in this single-file codebase, verified against Wikipedia and the US Naval Observatory through 2031. For v2 specifically, that constraint doesn't apply the same way, a real Python Integration has no reason to lean on a table that needs revisiting every decade or so. `v2-pagan-support` (`V2_PAGAN_SUPPORT.md`, `solstice_calc.py`) implements the actual Meeus periodic-term algorithm instead, the same general lineage as the Easter calculation already trusted in v1, tested against 64 calendar-day checks and 48 independent minute-precision checks, all passing. Permanent answer, no table to ever extend again.

The design question about single-evening versus extended sacred-night treatment got resolved too, as a real toggle rather than a unilateral pick either direction: off by default, standard single evening like any other holiday, on for the sacred-night treatment POW/MIA and Memorial Day already get, running until about 1am. Whoever's actually using it decides, not the codebase.

One genuine, real gap this surfaced that the original plan didn't anticipate: a floating pagan date can land on the exact same day as an existing holiday in a given year, 2026's Litha and Father's Day both resolve to June 21st. `V1_COLLISION_HANDLING.md` and the resulting `resolveTierWinner()` mechanism handle this now, Tonight's Pick generalized to work across any single-winner tier, not just the awareness-month split it was originally scoped for, plus a saved-preference data structure for permanent collisions like Samhain and Halloween, which land on the same date every single year. The settings screen to actually set that saved preference doesn't exist yet, that's the one real piece still open.
